...
const forwardHeaders = (res, inputHeaders, acceptHeaders = responseHeaders) =>
  ...
  inputHeaders.forEach((value, name) => {
    ...
    if (acceptHeaders.indexOf(name) > -1) {
      ...
      if (name === 'set-cookie') {
        cookieParser
          .parse(cookieParser.splitCookiesString(value))
          .forEach(({name, value, maxAge, ...options}) => {
            name = name.trim()
...
export default ({res, accept}) =>
  new ApolloLink((operation, forward) =>
    forward(operation).map(response => {
      ...
      if (headers) forwardHeaders(res, headers, accept)
