...
function r (plugin, options, isOptional, isEnabledByDefault = true) {
  const dep = `[req('${plugin}'), ${options || '{}'}]`

  if (isOptional) {
    if (isEnabledByDefault) {
      return `opt.${isOptional} === false ? {} : ${dep},`
    }
    else {
      return `opt.${isOptional} === true ? ${dep} : {},`
    }
  }
...
function run (dependencies) {
  ...
  function buildDep (key, dep, env) {
    ...
    const req = r(key, dep.options, dep.isOptional, dep.isEnabledByDefault)
